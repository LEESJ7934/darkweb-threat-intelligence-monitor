...
| Access control | 비인가 분석 화면 조회·과도한 관리자 권한 | auth 기본 True, 운영 모드 강제, POST 로그아웃·CSRF, Analyst/Security Admin 최소 권한분리, 단일 Django 진입점 유지 | [settings.py](../DjangoProject/DjangoProject/settings.py), [django_controls.py](django_controls.py), [tests_governance.py](../DjangoProject/mongoDbConnect/tests_governance.py), 중복 진입점 제거 | Django `is_staff` 기반 최소 RBAC이며 MFA/외부 SSO/정기 권한검토 미구현 |
...
| Audit review | 로그는 남지만 이상징후 검토가 누락 | 최근 30일 current/rotated JSONL을 read-only 집계하고 로그인실패·접근거부·오류·retention 오류·malformed 임계치를 판정 | [audit_review.py](audit_review.py), [review_audit.py](../scripts/review_audit.py), [Django tests](../DjangoProject/mongoDbConnect/tests_governance.py) | 실제 조직의 승인·보고 workflow 및 자동 SIEM alert는 미구현 |
...
| Backup/destructive protection | 복구 불능·resume 상태 손상 | 기존 토큰·볼륨 유지, drop/ES DELETE/자동 cascade 없음, 실행 전 범위·백업 검토 절차 | [retention.py](retention.py), [ELK 통합 운영 절차](../elk/operations.md), [data_handling.md](data_handling.md) | 자동 백업/복원 검증을 추가하지 않음; export/Telegram 과거 메시지 삭제 안 함 |
...
| Testing | 보안 통제 우회·기존 기능 회귀 | fake Mongo, 인증용 in-memory SQLite, 신규/기존 Python·JS 테스트 | [test_governance.py](../tests/test_governance.py), [Django 테스트](../DjangoProject/mongoDbConnect/tests_governance.py), [기존 ELK 테스트](../tests/test_elk.py) | Atlas/Tor/Telegram/ELK/Docker 실제 E2E는 Work에서 수행하지 않음 |
